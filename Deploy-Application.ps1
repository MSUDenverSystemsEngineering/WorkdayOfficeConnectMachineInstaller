...
Try {
	...
	[string]$appVendor = 'Workday'
	[string]$appName = 'OfficeConnect'
	[string]$appVersion = '2022.213.701.318'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '2.0'
	[string]$appScriptDate = '01/19/2023'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Show-InstallationWelcome -CloseApps 'excel,winword,powerpnt' -CheckDiskSpace -PersistPrompt -CloseAppsCountdown 60
		...
		## Please note: This installs OfficeConnect machnine-wide.
		Write-Log -Message "Attempting to run installer..." -Source 'Installation' -LogType 'CMTrace'
		$exitCode = Execute-Process -Path "$dirFiles\OfficeConnectMachineSetup.exe" -Parameters "/quiet" -WindowStyle "Hidden" -PassThru
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'excel,winword,powerpnt' -CloseAppsCountdown 60
		...
		$exitCode = Execute-Process -Path "$dirFiles\OfficeConnectMachineSetup.exe" -Parameters "/quiet /uninstall /norestart" -WindowStyle "Hidden" -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }

		#Can't get this one to work says the product is not installed
		#Write-Log -Message "Attempting to run Workday Event Log Components uninstaller..." -Source 'Uninstallation' -LogType 'CMTrace'
		#$exitCode = Remove-MSIApplications -Name "Workday Event Log Components"
		#If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
		...
		If (-not $useDefaultMsi) { Show-InstallationPrompt -Message "$appName $appVersion has been successfully uninstalled." -ButtonRightText 'OK' -Icon Information -NoWait }
	...
	ElseIf ($deploymentType -ieq 'Repair')
	{
		...
		Write-Log -Message "Attempting to run repair..." -Source 'Repair' -LogType 'CMTrace'
		$exitCode = Execute-Process -Path "$dirFiles\OfficeConnectMachineSetup.exe" -Parameters "/quiet /repair /norestart" -WindowStyle "Hidden" -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
		...
		If (-not $useDefaultMsi) { Show-InstallationPrompt -Message "$appName $appVersion has been successfully repaired." -ButtonRightText 'OK' -Icon Information -NoWait }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAJxAtC1J3OaZ/3
# bu7PZhK57Ge3REfFAfEG0ZXuJ+eyTKCCH8EwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# AYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgtnNj781r8Tkq
# 0ADFnQrNyOHonu0+1a2mdR4nl2dsK3kwDQYJKoZIhvcNAQEBBQAEggGACGmRpC28
# niskGu8dmgJylBHg3MOe/VA6gVZS3xgVSvpbYkSMxrDL73GkejRdpkQk4+FPuz33
# DhR5upuvG7j0X91pMm+HZjZ5ao7qxqfDdim9dpuoUiaECm/rEM7JwFNPAP5co/62
# z4vTmHa5mgpuSCwe4jYN/63TxWgJRLbpuhEl8PCrPkTmicAL8yv7vPo1spa1HCYx
# eoRJDQcLAyqPKG6SWA9n5HXLGyFmpxA+rFo3TG43DwGSv/Dj7bh7xyygb4uNSLeu
# /1yVu/RVv+0mwKRXzkW8hq4/xYjuAu2ALBjCUA40T8I6LDM4LFhvsgKl6IAiI+gI
# gSTB8yWZd6BuwvJKD63Gor4ODSNH+k4h4KC9odNuvWQUeeG2klY4espDSFp3tsPL
# W2aqhh79eqwBpv5IL/X/lwA5cT+cZ7IcQ0omQ89T6TUVfKkJ45P0dth6v7UQ01hW
# V55z7D+t2qRt9mC0hyyGD8EuUz/hu41P8ZfkDBk3LSrLTibw4QokqBIToYIDTDCC
...
# NzU3NTdaMD8GCSqGSIb3DQEJBDEyBDC7oDzMaKk+WxKQQkz+lUTH5nCOxIKaK8Z0
# Rb3lZf6UDPSodAveXKI86SdlRNulDnYwDQYJKoZIhvcNAQEBBQAEggIAKzjktf0G
# QqVxVbJ8FEmEVEobxx0slb2E7ZjMxJxYV4D1KWKfcfa7mZ1xXr4ynukXEMYyGO4p
# EqAxOZBCeDIMEhnD4hgDRAR2n7ZjIdEQL568Vj2/ivi1Bw3SDTlwwICU7Yxl8JHH
# CNrsLzobGyRsAW1uEZGQs/c+atUh9iJrsVmjmuXGwidkUNBwaLL7tu3YstppRAqp
# HQS444qCURVrorAzgcEl54NyIZb2flwxgZzM/w64bX66rET/I4rCZyvQubVNpvbF
# xST4FlM4CH1Z+WP+NWJaeTWK2Y4vGZIuRihDYYhWzfCki7nDiq4OqaGxuy2VI3MW
# j+QRhNEAiQdT7IJCpmbvU2g5/VuF8TLd6wBZImQQ0dJt/4EvrUxIMr4SAF4PmBEd
# YYqLiAmmV269xpT5AZnZ5xajIshulOB9GC6wmYpVfCmt6NbI/ROQZbxSNtTtg13h
# IHAsn46kp4snBr1+fiFtMKEUEkWMxFkuOYJGWLJU8HWtsEdClnWJsTiMXy7NgVMt
# hsMJpsBdNlL8RMtj+t4LCXIncAHxr1UH3wwXObafKwrGpTb0HWED3TrcH96ndTCR
# KZbxc9GINwcjR8l3wlwoOZzeB3WsN9Y0xQlPpIEecTvXirjW+79eFSWJBMM1dehg
# jzBU8+yZPCcRFKjBSPkaYgGgR3Yeg+9d3x8=
